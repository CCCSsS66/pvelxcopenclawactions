...
IMAGE_PREFIX="${IMAGE_PREFIX:-hermes-agent-root-lxc-debian12-amd64}"
...
IMAGE_NAME="${IMAGE_PREFIX}-${DATE_TAG}.tar.gz"
IMAGE_PATH="$DIST/$IMAGE_NAME"

if [ -n "$RELEASE_TAG_INPUT" ]; then
  RELEASE_TAG="$RELEASE_TAG_INPUT"
else
  RELEASE_TAG="hermes-root-lxc-${DATE_TAG}"
fi
...
log "[release] Publish GitHub Release"

if [ "${GITHUB_ACTIONS:-}" = "true" ] && [ "${PUBLISH_RELEASE:-true}" = "true" ]; then
  if ! command -v gh >/dev/null 2>&1; then
    echo "GitHub CLI gh not found, trying apt install..."
    sudo apt-get update
    sudo apt-get install -y gh
  fi

  if [ -z "${GITHUB_REPOSITORY:-}" ]; then
    echo "GITHUB_REPOSITORY is empty, skip release."
  elif [ -z "${GH_TOKEN:-}" ]; then
    echo "GH_TOKEN is empty, skip release."
  else
    ...
    if gh release view "$RELEASE_TAG" --repo "$GITHUB_REPOSITORY" >/dev/null 2>&1; then
      ...
      gh release create "$RELEASE_TAG" \
        ...
        "$DIST"/build-info.txt \
        "$DIST"/README.md \
        --title "Hermes Agent Root LXC ${RELEASE_TAG}" \
        --notes-file "$DIST/README.md" \
        --latest \
        --repo "$GITHUB_REPOSITORY"
    fi
  fi
else
  echo "PUBLISH_RELEASE=false or not running in GitHub Actions, skip release."
fi
